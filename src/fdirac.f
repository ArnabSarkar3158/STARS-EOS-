**==FDIRAC.F
      ...
      DIMENSION FF(4), GG(4), C(4,4,3), VW(4,4), VX(4,4)!C is a 4X4X3 matrix for elements in 5a EFF
     ...
     :     7.386560, 22.159680, 22.438048, 7.664928/!table 5a EFF
...
* derivatives w.r.t. T; the 3rd row logarithmic derivatives w.r.t. f
      ...
      UG = G*VG		!g/(1+g)
      FDF = G*G + G	!g^2+g 
      FDF = UF*FDF*SQRT(FDF)!f/(1+f)*(g^2+g)^1.5
      FF(1) = 1.0
      GG(1) = 1.0
      DO I = 2, NC
         FF(I) = F*FF(I-1) !FF(1)=1,FF(2)=F,FF(3)=F^2,FF(4)=F^3 
         GG(I) = G*GG(I-1) ! similarly for G
         FDF = FDF*VF*VG   !See the expressions above	
      END DO
      ND = 4
      DO I = 1, 3 !I is supposed to take care of 3rd dimension of C
         IF(I.GT.1) ND = 2
         DO L = 1, ND
            DO M = 1, ND + 1 - L
               VX(L,M) = 0.0	!Initializing all VX ==0
            END DO
         END DO			
         DO J = 1, NC		!1 to 4
            DO K = 1, NC	!1 to 4
               VW(1,1) = C(K,J,I)*GG(J)*FF(K) !Actual implementation (A_mn*f^m*g^n)
               DO L = 1, ND - 1
                  VW(L+1,1) = (J-1)*VW(L,1)
                  DO M = 1, ND - L
                     VW(L,M+1) = (K-1)*VW(L,M)
         ...
         IF(I.EQ.1) THEN	!Is I==1 corresponding to density?
* second and third order derivatives of density, needed in PRESSI
* !!! Programming should be checked!!!
            DTT = VX(3,1) - VX(2,1)**2 + (2.5 - NC)*UG*VG	!d2/dT2
            DFT = VX(2,2) - VX(2,1)*VX(1,2) + 0.5*UF*DTT	!d2/dFdT
            DFF = VX(1,3) - VX(1,2)**2 + UF*(DFT + (0.5*D(I,2) - NC)*VF !so on
     &           - 0.25*DTT*UF)
